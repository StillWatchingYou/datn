...
void transmitInterval() {
    ...
    if (offReceive == 1) {
        ...
        transmitString((uint8_t *)"\"Concern\": ");
        transmitString(_float_to_char(ppm, buf));

        transmitString((uint8_t *)", \n  ");
        transmitString((uint8_t *)"\"Hour\": ");
        transmitString(_float_to_char(hrtc1.date_time.hour, buf));

			  transmitString((uint8_t *)", \n  ");
        transmitString((uint8_t *)"\"min\": ");
        transmitString(_float_to_char(hrtc1.date_time.min, buf));
			
				transmitString((uint8_t *)", \n  ");
        transmitString((uint8_t *)"\"second\": ");
        transmitString(_float_to_char(hrtc1.date_time.second, buf));
